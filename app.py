...
class window_UI:
    def setupUi(self, window):
        ...
        self.folder_entry = Entry(
            window, width=28, font=("Times New Roman", 15), cursor="xterm"
        )
        self.folder_entry.place(x=170, y=10)

        # Open_Folder_Image
        self.img_file = (
            r"E:\Development\Python\My Project's\Files Renamer\folder_open.png"
        )
    ...
    def submit(self):
        ...
        for item in self.files_list:
            ...
            self.season = self.check_sea_ep(self.old_file_name, season_regex)
            self.episode = self.check_sea_ep(self.old_file_name, episode_regex)
    ...
    def check_sea_ep(file, regex):
        try:
            ...
            num_string = (
                f"0{num_string}"
                if len(str(num_string)) == 1 and int(num_string) < 10
                else num_string
            )
            return num_string
        except:
